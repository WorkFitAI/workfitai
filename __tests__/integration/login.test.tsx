...
// Mock auth-context login to simulate different responses
vi.mock('@/contexts/auth-context', () => ({
  useAuth: () => ({
    login: vi.fn().mockImplementation(async () => {
      // The real login calls authService.login; we test error-handling in the form here
    }),
    user: null,
...
async function fillLogin(usernameOrEmail: string, password: string) {
  ...
  await user.type(screen.getByPlaceholderText(/email address/i), usernameOrEmail)
...
describe('[D] Login Form', () => {
  it('D4 — empty form → Zod inline errors, no API call', async () => {
    const spy = vi.fn()
    server.use(http.post('http://localhost:9085/auth/login', () => { spy(); return HttpResponse.json({}) }))
    renderLoginForm()
    const user = userEvent.setup()
    await user.click(screen.getByRole('button', { name: /login/i }))
    await waitFor(() => {
  ...
  it('D10 — loading spinner visible during submit', async () => {
    // Use a delayed handler to catch the in-flight state
    server.use(
      http.post('http://localhost:9085/auth/login', async () => {
