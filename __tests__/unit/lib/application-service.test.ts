  ...
  it('default params — sends page=0, size=20, DESC sort', async () => {
    ...
    server.use(
      http.get(`${API}/application/my`, ({ request }) => {
        ...
        return apiSuccess({ items: [mockApplication()], meta: mockPaginationMeta() })
      })
    )
...
describe('getApplicationCount', () => {
  it('calls /application/my/count and returns totalApplications', async () => {
    server.use(
      http.get(`${API}/application/my/count`, () => apiSuccess({ totalApplications: 7 }))
    )
  ...
  it('returns applied:true with applicationId when already applied', async () => {
    server.use(
      http.get(`${API}/application/check`, () =>
        apiSuccess({ applied: true, applicationId: 'app-001', status: 'APPLIED' })
      )
    )
  ...
  it('calls /application/{id}/notes and returns notes', async () => {
    server.use(
      http.get(`${API}/application/app-001/notes`, () =>
        apiSuccess({ notes: [{ id: 'n1', author: 'hr', content: 'Good candidate', createdAt: '2026-01-15T08:00:00Z' }] })
      )
    )
...
describe('submitApplication', () => {
  it('calls apiClient.upload with correct FormData fields', async () => {
    ...
    const uploadSpy = vi.spyOn(apiClient, 'upload').mockResolvedValueOnce(
      { success: true, message: 'OK', data: mockData } as never
    )
    const file = new File(['%PDF-1.4'], 'resume.pdf', { type: 'application/pdf' })
    const result = await applicationService.submitApplication('job-001', 'test@example.com', file, 'Great cover letter')

    expect(uploadSpy).toHaveBeenCalledWith('/application', expect.any(FormData))
  ...
  it('omits coverLetter field when not provided', async () => {
    const uploadSpy = vi.spyOn(apiClient, 'upload').mockResolvedValueOnce(
      { success: true, message: 'OK', data: mockSubmitApplicationData() } as never
    )
    const file = new File(['%PDF-1.4'], 'resume.pdf', { type: 'application/pdf' })
    await applicationService.submitApplication('job-001', 'test@example.com', file)
...
describe('downloadCv', () => {
  it('throws when response is not ok', async () => {
    ...
    await expect(applicationService.downloadCv('app-001')).rejects.toThrow('CV download failed (403)')
  })
})
