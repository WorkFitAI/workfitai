...
import { useAdminUsers, useApprovalQueue, useAdminUser } from "@/hooks/useAdminUsers"
import type {
  EsSearchResult,
  EsUserHit,
  AdminUserSummary,
...
describe("useAdminUsers", () => {
  ...
  it("should build correct ES search body with pagination", async () => {
    ...
    renderHook(() =>
      useAdminUsers({ keyword: "john", page: 2, pageSize: 5 })
    )
...
describe("useAdminUser", () => {
  ...
  it("should toggle block for ACTIVE user", async () => {
    ...
    server.use(
      ...
      http.put(
        `${API}/user/admins/users/user-1/block`,
        async ({ request }) => {
          blockCalled = true
          blockValue = new URL(request.url).searchParams.get("blocked") === "true"
          return apiSuccess({})
        }
      )
    )
