...
import { apiSuccess, apiError, mockApplicationDetail, mockStatusHistoryItem } from '../../mocks/handlers'
...
describe('useApplicationDetail', () => {
  it('skips fetch when applicationId is empty', async () => {
    let callCount = 0
    server.use(
      http.get(`${API}/application/:id`, () => { callCount++; return apiSuccess(mockApplicationDetail()) }),
    )
  ...
  it('starts with loading=true then false after resolve', async () => {
    server.use(
      http.get(`${API}/application/app-001`, () => apiSuccess(mockApplicationDetail())),
  ...
  it('fetches detail and history in parallel and sets state', async () => {
    const detail = mockApplicationDetail({ id: 'app-001', status: 'INTERVIEW' })
    const history = [
      mockStatusHistoryItem({ newStatus: 'APPLIED', previousStatus: null }),
      mockStatusHistoryItem({ newStatus: 'INTERVIEW', previousStatus: 'APPLIED' }),
    ]
    server.use(
  ...
  it('re-fetches when applicationId changes', async () => {
    ...
    const { result, rerender } = renderHook(({ id }) => useApplicationDetail(id), {
