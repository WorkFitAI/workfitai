...
beforeEach(() => vi.spyOn(applicationService, 'downloadCv').mockResolvedValue(undefined))
afterEach(() => { server.resetHandlers(); vi.restoreAllMocks() })
afterAll(() => server.close())

function setupHandlers(
  detail = mockApplicationDetail(),
  history: ReturnType<typeof mockStatusHistoryItem>[] = [mockStatusHistoryItem()]
) {
  server.use(
    http.get(`${API}/application/${detail.id}`, () => apiSuccess(detail)),
    http.get(`${API}/application/${detail.id}/history`, () => apiSuccess(history)),
  )
}
...
describe('ApplicationDetailClient', () => {
  ...
  it('renders status history count badge and entries', async () => {
    const history = [
      ...
      mockStatusHistoryItem({ newStatus: 'REVIEWING', previousStatus: 'APPLIED', changedBy: 'hr@acme.com' }),
    ]
  ...
  it('calls downloadCv when CV filename button is clicked', async () => {
    ...
    await waitFor(() =>
      expect(applicationService.downloadCv).toHaveBeenCalledWith(detail.id, 'my-resume.pdf')
    )
  })
  ...
  it('"Back to My Applications" link points to /applied-jobs', async () => {
    ...
    render(<ApplicationDetailClient applicationId="app-001" />)
    await waitFor(() => expect(screen.getByText(/back to my applications/i)).toBeInTheDocument())
