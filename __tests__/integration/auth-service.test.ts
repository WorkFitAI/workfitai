...
describe("[D] Login — success paths", () => {
  ...
  it("D — login POST called with correct creds", async () => {
    ...
    expect(mockPost).toHaveBeenCalledWith(
      "/auth/login",
      expect.objectContaining({ usernameOrEmail: "user@test.com" }),
    );
  });
});
...
describe("[D] Login — error paths", () => {
  it("D5 — wrong password (400) → throws", async () => {
    ...
    await expect(
      authService.login({ usernameOrEmail: "u", password: "wrong" }),
  ...
  it("D7 — pending approval (403) → throws", async () => {
    mockPost.mockRejectedValueOnce(
      new ApiError("Account pending approval", 403),
    );
    await expect(
      authService.login({ usernameOrEmail: "hr", password: "p" }),
    ).rejects.toThrow("Account pending approval");
  });

  it("D9 — rate limited (429) → throws", async () => {
    mockPost.mockRejectedValueOnce(
      new ApiError("Too many login attempts", 429),
    );
    await expect(
      authService.login({ usernameOrEmail: "u", password: "p" }),
...
describe("[B] HR — WAIT_APPROVED", () => {
  ...
  it("B3 — HR login while pending (403) → throws", async () => {
    mockPost.mockRejectedValueOnce(
      new ApiError("Account pending approval by HR Manager", 403),
    );
    await expect(
      authService.login({
        usernameOrEmail: "hr@test.com",
...
describe("[E] Forgot Password — service layer", () => {
  it("E1 — forgotPassword called with correct endpoint + body", async () => {
    mockPost.mockResolvedValueOnce({
      success: true,
      data: { message: "OTP sent", expiresIn: 1800 },
    });
  ...
  it("E5 — verifyResetOtp returns resetToken", async () => {
    mockPost.mockResolvedValueOnce({
      success: true,
      data: { resetToken: "reset-tok-123", expiresIn: 1800 },
    });
  ...
  it("E6 — invalid OTP (400) → throws", async () => {
    ...
    await expect(
      authService.verifyResetOtp({ email: "user@test.com", otp: "000000" }),
  ...
  it("E9 — resetPassword called with correct body", async () => {
    ...
    expect(mockPost).toHaveBeenCalledWith(
      "/auth/reset-password",
      expect.objectContaining({ resetToken: "tok" }),
    );
  });
...
describe("[A] Registration — service layer", () => {
  ...
  it("A9 — invalid OTP (400) → throws", async () => {
    mockPost.mockRejectedValueOnce(new ApiError("Invalid or expired OTP", 400));
    await expect(
      authService.verifyOtp({ email: "u@test.com", otp: "000000" }),
    ).rejects.toThrow("Invalid or expired OTP");
  });
});
