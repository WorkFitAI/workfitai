...
describe('AppliedJobsPageClient', () => {
  ...
  it('renders application cards and total count after load', async () => {
    ...
    server.use(
      http.get(`${API}/application/my`, () =>
        apiSuccess({ items: [app], meta: mockPaginationMeta({ totalElements: 1 }) })
      )
    )
    render(<AppliedJobsPageClient />)
    await waitFor(() => expect(screen.getByText('1 application total')).toBeInTheDocument())
  ...
  it('shows empty state message when no applications exist', async () => {
    ...
    render(<AppliedJobsPageClient />)
    await waitFor(() => expect(screen.getByText('0 applications total')).toBeInTheDocument())
  ...
  it('pagination renders when totalPages > 1', async () => {
    server.use(
      http.get(`${API}/application/my`, () =>
        apiSuccess({
          items: Array.from({ length: 10 }, (_, i) => mockApplication({ id: `app-${i}` })),
          meta: mockPaginationMeta({ totalPages: 3, totalElements: 30 }),
        })
      )
    )
    render(<AppliedJobsPageClient />)
    await waitFor(() => expect(screen.getByText('30 applications total')).toBeInTheDocument())
  ...
  it('shows correct plural for multiple applications', async () => {
    ...
    render(<AppliedJobsPageClient />)
    await waitFor(() => expect(screen.getByText('2 applications total')).toBeInTheDocument())
  })
})
