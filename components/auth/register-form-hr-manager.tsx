...
import { hrManagerRegisterSchema, type HrManagerRegisterFormValues } from '@/lib/schemas/auth-schemas'
...
export function RegisterFormHrManager() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<HrManagerRegisterFormValues>({ resolver: zodResolver(hrManagerRegisterSchema) })
  ...
  async function onSubmit(data: HrManagerRegisterFormValues) {
    ...
    try {
      const response = await authService.register({
        ...
        role: 'HR_MANAGER',
        hrProfile: { department: data.department, hrManagerEmail: data.email, address: data.address },
        company: {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1">
          ...
          <Input id="hrm-fullName" placeholder="Jane Doe" {...register('fullName')} />
          {errors.fullName && <p className="text-sm text-destructive">{errors.fullName.message}</p>}
        </div>
        <div className="space-y-1">
          <Label htmlFor="hrm-phone">Phone</Label>
          <Input id="hrm-phone" type="tel" placeholder="+1234567890" {...register('phoneNumber')} />
      ...
      <div className="space-y-1">
        ...
        <Input id="hrm-email" type="email" placeholder="you@company.com" {...register('email')} />
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1">
          ...
          <Input id="hrm-address" placeholder="123 Main St" {...register('address')} />
          {errors.address && <p className="text-sm text-destructive">{errors.address.message}</p>}
        </div>
      </div>

      <PasswordInput id="hrm-password" label="Password" error={errors.password?.message} {...register('password')} />
      <PasswordInput id="hrm-confirm" label="Confirm Password" error={errors.confirmPassword?.message} {...register('confirmPassword')} />
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1">
          ...
          <Input id="hrm-co-name" placeholder="Acme Corp" {...register('companyName')} />
          {errors.companyName && <p className="text-sm text-destructive">{errors.companyName.message}</p>}
        </div>
        <div className="space-y-1">
          <Label htmlFor="hrm-co-size">Company Size</Label>
          <Input id="hrm-co-size" placeholder="50-200" {...register('companySize')} />
        </div>
      </div>

      <div className="space-y-1">
        <Label htmlFor="hrm-co-address">Company Address</Label>
        <Input id="hrm-co-address" placeholder="456 Business Ave" {...register('companyAddress')} />
      ...
      <div className="space-y-1">
        ...
        <Input id="hrm-co-web" type="url" placeholder="https://acme.com" {...register('companyWebsite')} />
